...
function ex03_enc($frase, $cifra) {
    ...
    for ($i = 0; $i < strlen($frase); $i++) {
        $position = find($values_shuffled, $frase[$i]);
        if ($position !== FALSE) {
            $new = $position + $cifra;
            while ($new >= $n_values) {
                $new -= $n_values;
...
function ex07($data1, $data2) {
    if (ex05($data1) == 0 || ex05($data2) == 0) {
        echo "Data introduzida incorrectamente!!";
        return 0;
    }
